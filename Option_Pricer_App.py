...
class Option:
    ...
    def plot_call(self, option_position, spot_values, call_price=None):
        ...
        fig_call.add_vline(
            ...
            annotation_text=f"Breakeven: {breakeven_call:.2f}",
    ...
    def plot_put(self, option_position, spot_values, put_price=None):
        ...
        fig_put.add_vline(
            ...
            annotation_text=f"Breakeven: {breakeven_put:.2f}",
            annotation_position="top left",
            annotation_font=dict(color="orange")
        )
    ...
    def plot_call_spread(self, option_position, spot, K1, K2, maturity, rate, div, sigma):
        ...
        fig.add_vline(x=breakeven, line_dash="dot", line_color="orange", annotation_text=f"Breakeven: {breakeven:.2f}",
...
if input_mode == "Manual Inputs":
    ...
    if calculated_maturity < 0.01:
        st.warning("To calculate the theoretical price using the Black-Scholes model, a very small maturity can cause numerical instability or division by zero. Since the entered maturity is very close to 0, we set a minimum maturity threshold of 0.01 years (approximately one day) to ensure stability in the calculations.")
...
with tab6: 
    ...
    with col1:
        purchase_price_call = st.number_input(
            "Purchase Call Price",
            min_value=0.0,
            value = call_price if input_mode == "Manual Inputs" else float(mkt_call_price),
            step=0.01,
    ...
    with col2:
        purchase_price_put = st.number_input(
            "Purchase Put Price",
            min_value=0.0,
            value = put_price if input_mode == "Manual Inputs" else float(mkt_put_price),
            step=0.01,
            help="The price at which you bought the put"
    )
    
    # Display PnL results
    st.header("PnL Results")
